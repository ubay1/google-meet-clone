...
const SetupV3: FC<{ params: string }> = ({ params }) => {
  ...
  // panggil ke room
  const callToPeerRoom = () => {
    setTimeout(() => {
      if (!ref1.current.srcObject) {
        console.error('No media stream available to call.')
        return
      }
      const call = peerUser.call(roomName, ref1.current.srcObject)
      if (!call) {
  ...
  function onConnect() {
    ...
    // console.log('peerRoom = ', peerRoom)

    setTimeout(() => {
      const conns = peerUser.connect(roomName)
      conns.on('open', () => {
        console.log('Menghubungkan ke room ' + roomName)
        setIsConnectedToRoom(true)
      })
      conns.on('data', (data: any) => {
        console.log('Pesan dari room: ', data)
      })
    }, 1000)
  }
  ...
  useEffect(() => {
    ...
    setPeerRoom(peer)
    console.log('peer room jalan di mounted setup: ', peer)

    peer.on('connection', (conn: any) => {
      setTimeout(() => {
        conn.send(`hello, kamu sudah konek ke room ${params}`)
      }, 1000)
    })
  ...
  // set peerUser from peerRoom
  useEffect(() => {
    if (peerRoom) {
      peerRoom.on('open', (id: any) => {
        console.log('Peer ID Room adalah: ' + id)
      })
      ...
      peerUser.on('open', (id: any) => {
        console.log('Peer ID kamu adalah: ' + id)
        setAllMediaStream()
  ...
  useEffect(() => {
    if (peerUser) {
      ...
      setTimeout(() => {
        ...
        conn.on('open', () => {
          console.log('Connected ke room: ' + peerRoom)
        })
          ...
          >
            ...
            <Flex gap="4">
              ...
              <Button
                ...
                radius="full"
                className="cursor-pointer disabled:cursor-not-allowed w-auto"
                disabled={isEmpty(name) || isEmpty(roomName) || !isConnectedToRoom}
                onClick={finishSetup}
              >
                Gabung sekarang
              </Button>
