...
export default function App() {
  ...
  const [statusPermissionCamera, setStatusPermissionCamera] = useState<statusPermissions>("belum-aktif");
  const [statusPermissionMicrophone, setStatusPermissionMicrophone] = useState<statusPermissions>("belum-aktif");
  ...
  const [openDialogPermissionCameraDenied, setOpenDialogPermissionCameraDenied] = useState<boolean>(false);
  const [openDialogPermissionMicrophoneDenied, setOpenDialogPermissionMicrophoneDenied] = useState<boolean>(false);
    ...
    try {
      ...
      listMedia.forEach((device: any) => {
        if (device.kind === "audioinput" && !isEmpty(device.deviceId)) {
          ...
          setListMicrophone((allData) => [...allData, { deviceId: device.deviceId, label: device.label }]);
        } else if (device.kind === "audiooutput" && !isEmpty(device.deviceId)) {
          if (device.deviceId === "default") {
            setSpeakerType({ deviceId: device.deviceId, label: device.label });
          }
          setListSpeaker((allData) => [...allData, { deviceId: device.deviceId, label: device.label }]);
        } else if (device.kind === "videoinput" && !isEmpty(device.deviceId)) {
          setCameraType({ deviceId: device.deviceId, label: device.label });
          setListCamera((allData) => [...allData, { deviceId: device.deviceId, label: device.label }]);
        }
      });
    } catch (error) {
    ...
    <Box className='h-screen w-full'>
      <Flex justify='center' direction='column' gap='4' px='4' align='center' height='100%'>
        ...
        >
                  ...
                  >
                    <div className='grid grid-cols-[auto_calc(100%-20px)] gap-2'>
                      ...
                      <div className='w-auto truncate'>
                        {statusPermissionMicrophone !== "aktif" ? "Izin diperlukan" : selectMicrophoneType?.label}
                      </div>
                    </div>
                  </Select.Trigger>
                ...
                >
                  ...
                  >
                    <div className='grid grid-cols-[auto_calc(100%-20px)] gap-2'>
                      ...
                      <div className='w-auto truncate'>
                        {statusPermissionMicrophone !== "aktif" ? "Izin diperlukan" : selectSpeakerType?.label}
                      </div>
                    </div>
                  </Select.Trigger>
                  <Select.Content position='popper'>
          ...
          <Box width={{ initial: "100%", md: "30%" }} className='flex flex-col justify-center items-center gap-4'>
      ...
      <Flex>
        <Dialog.Root open={openDialogPermissionCameraDenied || openDialogPermissionMicrophoneDenied}>
